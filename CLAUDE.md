...
- **Prod domain (planned):** `danieloliverracing.com` — not yet attached in Netlify. The code already treats it as canonical (`astro.config.mjs` `site`, and `site_url`/`display_url` in `config.yml`), so launch = add the custom domain in Netlify. One code touch-up at launch: `publicOrigin` in `BaseLayout.astro` (used for `og:image`/`og:url` so social share previews resolve on a live host) is currently the `.netlify.app` URL — point it at the custom domain once attached (the `.netlify.app` URL keeps working regardless).
- **Social share card:** default OG/Twitter image is `public/images/og/og-default.jpg` (1200×630 branded card). It was generated by screenshotting a temporary `og-card.astro` page with Playwright — re-create that page and re-shoot if the card needs updating (emoji flags / webp don't preview reliably, hence a baked JPG). Per-page covers (posts/galleries) override it via the `ogImage` prop.
