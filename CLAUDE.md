...
- **Track maps + flags** (`TrackMap.astro`, `src/lib/tracks.ts`, `src/data/tracks.json`): events **and news/blog posts** have an optional `track` id (CMS dropdown, 222 options). `trackById()` resolves it to a country flag + a projected SVG circuit outline. Events show it as a corner overlay + flag in the meta line; posts show an outline+flag+name panel in the header (`news/[slug].astro`, `blog/[slug].astro`). `tracks.json` (222 tracks, ~34 with drawn maps) is regenerated from the sibling `track-manager-pro` repo via `node scripts/build-tracks.mjs` — don't hand-edit it. `TrackMap` defaults to a corner overlay: it sets `position:absolute` inline on purpose (the global `.card > *{position:relative}` rule has equal specificity to Tailwind's `.absolute` and would otherwise win by source order — same reason the checkered wash div carries an inline `position:absolute`). Pass **`inline`** to render it in normal flow (used by the post header panel).
